...
/**
 * Created by jcardozo  on 6/10/18.
 */

public class ReadServerActivity extends BaseActivity implements View.OnClickListener {
    ...
    protected void onCreateView(Bundle savedInstanceState, ViewGroup container) {
        ...
        sendMessage= (TextView) root.findViewById(R.id.sendMessage);
        sendMessage.setText("Paste the URI here to set profile server");
        root.findViewById(R.id.img_qr).setOnClickListener(this);
        btn_add = (Button) root.findViewById(R.id.btn_add);
        btn_add.setText("Read Server");
                            ...
                            try {
                                ...
                                future.setListener(new BaseMsgFuture.Listener<ProfileInformation>() {
                                    ...
                                    public void onAction(int messageId, ProfileInformation object) {
                                        runOnUiThread(new Runnable() {
                                            ...
                                            public void run() {
                                                ...
                                                Snackbar.make(v, R.string.server_read, Snackbar.LENGTH_LONG).show();
                                                progressBar.setVisibility(View.INVISIBLE);
                                                enableSendBtn();
                                            }
                                        });
                                    ...
                                    public void onFail(int messageId, final int status, final String statusDetail) {
                                        Log.i("ReadServer", "trying to read server" + statusDetail);
                                        runOnUiThread(new Runnable() {
                                            @Override
                                            public void run() {
                                                Log.i(TAG, "trying to read server, " + statusDetail);
                                                Snackbar.make(v, getString(R.string.reading_server_trying), Snackbar.LENGTH_LONG).show();
                                                progressBar.setVisibility(View.INVISIBLE);
                                                enableSendBtn();
                                            }
                                        });
                                ...
                                runOnUiThread(new Runnable() {
                                    @Override
                                    public void run() {
                                        Log.i(TAG, "Server read fail" + e.getMessage());
                                        Snackbar.make(v, R.string.server_read_fail, Snackbar.LENGTH_LONG).show();
                                        enableSendBtn();
                                        progressBar.setVisibility(View.INVISIBLE);
                                    }
                                });
        ...
        if (requestCode == SCANNER_RESULT) {
            if (resultCode == RESULT_OK) {
                try {
                    ...
                    Toast.makeText(this, "Bad URI", Toast.LENGTH_LONG).show();
                }
            }
        }
